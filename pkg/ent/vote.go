...
func (*Vote) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case vote.ForeignKeys[0]: // content_votes
	...
	for i := range columns {
		...
		case vote.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				return fmt.Errorf("unexpected type %T for field content_votes", values[i])
			} else if value.Valid {
				v.content_votes = new(uuid.UUID)
				*v.content_votes = *value.S.(*uuid.UUID)
			}
