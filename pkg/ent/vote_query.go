...
func (vq *VoteQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Vote, error) {
	...
	if vq.withContent != nil || vq.withUser != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := vq.withContent; query != nil {
		if err := vq.loadContent(ctx, query, nodes, nil,
			func(n *Vote, e *Content) { n.Edges.Content = e }); err != nil {
			return nil, err
		}
	}
...
func (vq *VoteQuery) loadContent(ctx context.Context, query *ContentQuery, nodes []*Vote, init func(*Vote), assign func(*Vote, *Content)) error {
	...
	for i := range nodes {
		if nodes[i].content_votes == nil {
			continue
		}
		fk := *nodes[i].content_votes
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "content_votes" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
