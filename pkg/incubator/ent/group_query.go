...
func (gq *GroupQuery) QueryInvites() *GroupInviteQuery {
	query := (&GroupInviteClient{config: gq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GroupQuery) QueryContent() *ContentQuery {
	query := (&ContentClient{config: gq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withContent:    gq.withContent.Clone(),
...
func (gq *GroupQuery) WithInvites(opts ...func(*GroupInviteQuery)) *GroupQuery {
	query := (&GroupInviteClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (gq *GroupQuery) WithContent(opts ...func(*ContentQuery)) *GroupQuery {
	query := (&ContentClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gq.withContent = query
	return gq
}
...
func (gq *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := gq.withInvites; query != nil {
		if err := gq.loadInvites(ctx, query, nodes,
			func(n *Group) { n.Edges.Invites = []*GroupInvite{} },
			func(n *Group, e *GroupInvite) { n.Edges.Invites = append(n.Edges.Invites, e) }); err != nil {
			return nil, err
		}
	}
	if query := gq.withContent; query != nil {
		if err := gq.loadContent(ctx, query, nodes,
			func(n *Group) { n.Edges.Content = []*Content{} },
			func(n *Group, e *Content) { n.Edges.Content = append(n.Edges.Content, e) }); err != nil {
			return nil, err
		}
	}
...
func (gq *GroupQuery) loadGroupUsers(ctx context.Context, query *GroupUserQuery, nodes []*Group, init func(*Group), assign func(*Group, *GroupUser)) error {
	...
	for _, n := range neighbors {
		fk := n.group_group_users
		if fk == nil {
			return fmt.Errorf(`foreign-key "group_group_users" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "group_group_users" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (gq *GroupQuery) loadInvites(ctx context.Context, query *GroupInviteQuery, nodes []*Group, init func(*Group), assign func(*Group, *GroupInvite)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.GroupInvite(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.InvitesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (gq *GroupQuery) loadContent(ctx context.Context, query *ContentQuery, nodes []*Group, init func(*Group), assign func(*Group, *Content)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(group.ContentTable)
		s.Join(joinT).On(s.C(content.FieldID), joinT.C(group.ContentPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(group.ContentPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(group.ContentPrimaryKey[1]))
