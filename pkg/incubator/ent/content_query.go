...
func (cq *ContentQuery) loadVotes(ctx context.Context, query *VoteQuery, nodes []*Content, init func(*Content), assign func(*Content, *Vote)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(content.VotesTable)
		s.Join(joinT).On(s.C(vote.FieldID), joinT.C(content.VotesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(content.VotesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(content.VotesPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
